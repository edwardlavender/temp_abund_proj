Modelling the impacts of climate change on the relative abundance of
...
***Figure 1. An overview of the methods used to predict change in the
relative abundance of shallow-water fish under future temperature
change.** We started with a list of over 2000 shallow-water fish
species. For each species, we overlaid a model of occurrence with
...
2.  `data` contains processed data.
      ...
      - `abundance` contains abundance predictions for SST and SBT for
        all temperature scenarios, generated by `project_abund_1.R`.  
        <br />
...
1.  `project_abund_1.R` predicts changes in species’ relative abundance
